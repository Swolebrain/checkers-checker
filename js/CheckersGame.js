export default class CheckersGame{
  ...
  validateMoves(){
    ...
    this.movesList.forEach((move, idx)=>{
      ...
      if (currentState[move.from.y+moveDirY][move.from.x-1] === opponentColor &&
      ...
      else if (currentState[move.from.y+moveDirY][move.from.x+1] === opponentColor &&
            currentState[move.from.y+2*moveDirY][move.from.x+2] === ""){
  ...
  determineWinner(){
    ...
    else if (blackPieces > whitePieces) return "black wins!";
